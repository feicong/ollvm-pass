...
void IndirectCall::run(Module &M) {
    ...
    for (Function *F : functions) {
        funcAddrs.push_back(ConstantExpr::getBitCast(F, PointerType::get(context, 0)));
    }

    // Save function addresses to global variable
    ArrayRef<Constant *> funcAddrsRef(funcAddrs);
    ArrayType *functionTableType = ArrayType::get(PointerType::get(context, 0), funcAddrs.size());
