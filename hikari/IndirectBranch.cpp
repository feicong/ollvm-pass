...
bool IndirectBranch::initialize(Module &M) {
  PassBuilder PB;
...
bool IndirectBranch::runOnFunction(Function &Func) {
  ...
  for (BranchInst *BI : BIs) {
    ...
    if (BI->isConditional()) {
      ...
      if (EncryptJumpTarget) {
        ...
        indexval = (UseStack ? IRBEntry : IRBBI)
      ...
      index = indexval;
      RealIndex = EncryptJumpTarget ? IRBBI->CreateXor(index, IndexEncKey)
                                        : index;
    }
...
void IndirectBranch::shuffleBasicBlocks(Function &F) {
