...
bool FunctionCallObfuscate::OnlyUsedByCompilerUsed(GlobalVariable *GV) {
  if (GV->getNumUses() == 1) {
    User *U = GV->user_back();
    if (U->getNumUses() == 1) {
...
void FunctionCallObfuscate::HandleObjC(Function *F) {
...
bool FunctionCallObfuscate::runOnFunction(Function &F) {
