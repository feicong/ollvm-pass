...
bool BogusControlFlow::runOnFunction(Function &F) {
  if (!F.isPresplitCoroutine() && !readAnnotationMetadata(&F, "bcfopfunc")) {
    bogus(F);
    doF(F);
    return true;
  }
...
bool BogusControlFlow::containsCoroBeginInst(BasicBlock *b) {
  for (Instruction &I : *b)
...
bool BogusControlFlow::containsMustTailCall(BasicBlock *b) {
  for (Instruction &I : *b)
...
bool BogusControlFlow::containsSwiftError(BasicBlock *b) {
  for (Instruction &I : *b)
...
/* createAlteredBasicBlock
  ...
  * behave nicely.
  */
BasicBlock *BogusControlFlow::createAlteredBasicBlock(BasicBlock *basicBlock, const Twine &Name, Function *F) {
  BasicBlock *alteredBB = OnlyJunkAssembly
...
/* doF
  ...
  * of the function.
  */
bool BogusControlFlow::doF(Function &F) {
